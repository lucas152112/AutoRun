...
public class SchedulerService : IDisposable
{
    ...
    private void TryStart(ScheduleItem item)
    {
        try
        {
            ...
            Process.Start(psi);
            OnRun?.Invoke(this, $"[{DateTime.Now:yyyy-MM-dd HH:mm}] ±Ò°Ê: {item.Name} -> {item.ExePath} {item.Arguments}");
        }
        catch (Exception ex)
        {
            OnRun?.Invoke(this, $"[{DateTime.Now:yyyy-MM-dd HH:mm}] ¥¢±Ñ: {item.Name} -> {ex.Message}");
        }
    }
