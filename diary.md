...
### 6. 修正啟動時顯示第一筆資料問題
- **問題**: 程式啟動時，雖然已禁用輸入欄位，但輸入欄位仍會顯示第一筆資料的內容
- **原因**: DataGridView 在資料綁定後會自動觸發 `SelectionChanged` 事件，導致第一筆資料被載入
- **解決方式**:
  - 在載入資料前，暫時移除 `SelectionChanged` 事件處理器
  - 載入完成後，呼叫 `ClearInputs()` 清空所有輸入欄位
  - 呼叫 `DisableInputs()` 禁用輸入欄位
  - 最後重新加入 `SelectionChanged` 事件處理器
- **效果**: 程式啟動時，輸入欄位完全空白且禁用，只有在使用者點擊「新增」或選擇 Grid 項目時才會啟用和顯示資料
